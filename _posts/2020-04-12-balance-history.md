...
However, I realized I could use the popular Python package [pandas](https://pandas.pydata.org/) to wrangle the transaction data and compute a balance history. Then I learned a visualization tool that I'd never used, but heard a lot of good things about, [Plotly](https://plotly.com/python/), to create an interactive plot of my balance history, including all the information I wanted to see. This included a line plot of the daily balance, with hover text displaying the description and amount of individual transactions upon mouseover, as well as a monthly average balance. In the end, I found the result quite helpful in giving me a quick and informative look at my financial history.
...
Aside from these specifications, the daily balance plot has additional options including the custom hover text we created (`text`), and a `hoverinfo` argument stating that we want the x- and y-coordinates displayed upon mouseover, along with the custom text. Finally this plot is given an `hv` line shape, which means that in order to trace the line from point to point through time, first a step is taken in the horizontal direction, then the vertical direction. This is appropriate because the balance remains the same between dates of transactions. If we were to plot the daily-interpolated balance we created to find the monthly averages, it would lie along this line, although I think the plot looks nicer as shown here.
